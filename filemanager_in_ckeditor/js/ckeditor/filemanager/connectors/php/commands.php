...
function DeleteFile( $resourceType, $currentFolder )
{
	...
	$sServerDir = ServerMapFolder( $resourceType, $currentFolder, 'DeleteFile' ) ;
	...
	{
		$sDeleteFileName = SanitizeFileName($_GET['DeleteFileName']) ;
	}
	if (file_exists($sServerDir.$sDeleteFileName)){
		unlink($sServerDir.$sDeleteFileName);
		echo '<Error number="' . '0' . '" />' ;
	}		
	echo '<Error number="102" />' ;
...
{	
	...
	// Map the virtual path to the local server path.
	$sServerDir = ServerMapFolder( $resourceType, $currentFolder, 'DeleteFolder' ) ;
	if (!isset($_GET)) {
		global $_GET;
	}
	if ( isset( $_GET['DeleteFolderName'] ) )
	{
		$sDeleteFolderName = SanitizeFolderName($_GET['DeleteFolderName']) ;
	
	}
	if ( deleteDir($sServerDir.$sDeleteFolderName)){		
		echo '<Error number="' . '0' . '" />' ;
	}		
	echo '<Error number="120" />' ;
	//echo '<Error number="' . $sServerDir.$sDeleteFolderName . '" />' ;
