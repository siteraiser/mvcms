...
*/	
class bs4pagination_model extends requestHandler{
	/*
...
*/
	...
	private $pagination_open='<ul class="pagination">';
	private $pagination_close='</ul>';
	private 	$open='<li class="page-item">';
	private 	$close='</li>';
	private $active_open='<li class="page-item active"><a class="page-link" href="#">';
	private $active_close='<span class="sr-only">(current)</span></a></li>';//<span class='sr-only'></span></a></li></li>
	private $next_open='<li class="page-item">';
	private $next_close="</li>";
	private $prev_open='<li class="page-item">';
	private $prev_close="</li>";	
		
	private $disabled_left='<li class="page-item disabled">
      <a class="page-link" href="#" aria-label="Previous">
        <span aria-hidden="true">&laquo;</span>
        <span class="sr-only">Previous</span>
      </a>
    </li>';	
	private $disabled_right='<li class="page-item disabled">
      <a class="page-link" href="#" aria-label="Previous">
	...
	private $prev_chevron_left='';
	private $next_chevron_right='';		
	private $first_open='<li class="page-item">';
	private $first_close="</li>";	
	private $last_open='<li class="page-item">';
	private $last_close="</li>";	
...
//Number and check for current page
	for($i=$start; $i<=$end && $i<=$total; $i++){
		if($i==$page_num && $i==$total){
			...
			$currentpage.=$this->open."<a class='page-link' href='".$this->link_url."?".$this->get_var."=$i".$this->link_params."'>$i</a>".$this->close;
		}
	}
...
//Work out prev and next links
	if($total>1 && $this->prevnext != false ){
		if($page_num=="1"){
			$page=$page_num + 1;
			$next=$this->next_open."<a rel='next' class='page-link' aria-label='Next' href='".$this->link_url."?".$this->get_var."=$page".$this->link_params."' >".$this->next_chevron_right."<span aria-hidden='true'>&raquo;</span><span class='sr-only'></span></a>".$this->next_close;
			$prev=$this->disabled_left;//bootstrap is ''
		}else if($page_num==$total){
			$page=$page_num - 1;
			$next=$this->disabled_right;//bootstrap is ''
			$prev=$this->prev_open."<a rel='prev' class='page-link' aria-label='Previous' href='".$this->link_url."?".$this->get_var."=$page".$this->link_params."' >".$this->prev_chevron_left."<span aria-hidden='true'>&laquo;</span><span class='sr-only'></span></a>".$this->prev_close;			
		}else{
			$next=$this->next_open."<a rel='next' class='page-link' aria-label='Next' href='".$this->link_url."?".$this->get_var."=". ($page_num + 1) .$this->link_params."' >".$this->next_chevron_right."<span aria-hidden='true'>&raquo;</span><span class='sr-only'></span></a>".$this->next_close;
			$prev=$this->prev_open."<a rel='prev' class='page-link' aria-label='Previous' href='".$this->link_url."?".$this->get_var."=". ($page_num - 1) .$this->link_params."' >".$this->prev_chevron_left."<span aria-hidden='true'>&laquo;</span><span class='sr-only'></span></a>".$this->prev_close;	
		}
	}else{
...
//Work out first and last links
	if($total>1 && $this->firstlast != false ){
		$firstlink='';
		$lastlink='';
		$lastlink=$this->last_open."<a class='page-link' href='".$this->link_url."?".$this->get_var."=$total".$this->link_params."'>Last</a>".$this->last_close;
		$lastlink.=$next;
		$firstlink=$prev.$this->first_open."<a class='page-link' href='".$this->link_url."?".$this->get_var."=1".$this->link_params."'>First</a>".$this->first_close;
	}
